...
def compute_features(rows):
    """Builds player_game_features rows. All *_prior values are computed
    over games strictly before the current row -- current row's own box
    score is never included. DNP games are excluded from every average
    but still get a features row (with null threshold flags)."""
    features = []
